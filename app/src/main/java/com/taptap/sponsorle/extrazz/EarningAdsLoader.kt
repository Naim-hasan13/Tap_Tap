...
class CustomAdLoader(
    ...
    private fun setupDialogViews(dialog: AlertDialog) {
        ...
        val skipBtn = dialog.findViewById<TextView>(R.id.iv_close_ad)
        val policy = dialog.findViewById<TextView>(R.id.iv_policy)
        val earnads = dialog.findViewById<TextView>(R.id.earnAdsImage)
...
class BannerAdLoader(
    ...
    fun loadBannerAd() {
        val urlforAds = "${siteAdsUrl}get_banner.php"
        val requestQueue: RequestQueue = Volley.newRequestQueue(context)
        ...
        val jsonArrayRequest = object : StringRequest(urlforAds, { response ->
            ...
            if (jsonArr.length() > 0) {
                ...
                videoView.setVideoURI(Uri.parse(earningAdBannerModel!!.ad_video_link))
                mute_unmute.setImageResource(if (isMuted) R.drawable.mute else R.drawable.un_mute)
                updateImpression()
                videoView.setOnCompletionListener {
                    videoView.stopPlayback()
                ...
                videoView.setOnErrorListener { _, what, extra ->
                    Toast.makeText(context, "Ads No Available !", Toast.LENGTH_SHORT).show()
                ...
                videoView.setOnPreparedListener { mp ->
                    ...
                    videoView.setOnTouchListener { _, _ ->
                        ...
                        if(earningAdBannerModel!!.click_link.contains("igya")){
                            updateClick()
                            Utils.openUrl(context, earningAdBannerModel!!.click_link)
                        }else if(earningAdBannerModel!!.click_link==""){
                            Toast.makeText(context, "Ads No Available !", Toast.LENGTH_SHORT).show()
                        }
    ...
    // Make sure to release the media player when it's no longer needed
    ...
    private fun updateImpression() {
        ...
        val stringRequest = object : StringRequest(Method.POST, url3, { response ->
            ...
            override fun getParams(): Map<String, String> {
                ...
                val ads_id32 =
                    videoplayyer.encrypt(earningAdBannerModel!!.ads_id, Hatbc()).toString()
                val uapp_id32 = videoplayyer.encrypt(uniqueAppId, Hatbc()).toString()
